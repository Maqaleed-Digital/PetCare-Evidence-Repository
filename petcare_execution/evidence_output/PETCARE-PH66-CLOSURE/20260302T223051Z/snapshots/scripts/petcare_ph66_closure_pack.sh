...
echo "=============================================="
echo "PetCare PH66 CLOSURE PACK"
...
need=(
  ...
  "scripts/petcare_verification_index_no_patch_guard.sh"
...
echo ""
echo "=== STEP 1: NO-PATCH GUARD STANDALONE (MUST PASS) ==="
bash "${REPO_ROOT}/scripts/petcare_verification_index_no_patch_guard.sh" \
  | tee "${OUT}/logs/no_patch_guard.log"
...
snap=(
  ...
  "scripts/petcare_verification_index_no_patch_guard.sh"
  "scripts/petcare_ci_gates.sh"
  "scripts/petcare_ph66_closure_pack.sh"
)
for f in "${snap[@]}"; do
