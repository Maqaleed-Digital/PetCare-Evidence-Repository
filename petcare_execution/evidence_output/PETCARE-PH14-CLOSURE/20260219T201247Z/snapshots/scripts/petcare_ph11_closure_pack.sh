...
echo "=============================================="
echo "PetCare PH11 CLOSURE PACK"
echo "pack=${PACK}"
...
echo "=== SNAPSHOT (PH11-RELEVANT FILES) ==="
REQ_FILES="
...
TESTS/test_ph11_http_adversarial.py
ops/ph11_gate_registry.json
scripts/petcare_ph11_enforce_gates.py
scripts/petcare_ph11_closure_pack.sh
"
...
echo "=== AUDIT VERIFY (DEMO INPUT) ==="
python3 "scripts/petcare_ph11_audit_verify.py" <<'JSON' 2>&1 | tee "${OUT}/ops/audit_verify.json"
{
  "events": []
}
JSON
...
echo "=== ENFORCE GATES ==="
python3 "scripts/petcare_ph11_enforce_gates.py" "ops/ph11_gate_registry.json" 2>&1 | tee "${OUT}/ops/gate_enforce.txt"

echo "=== CREATE MANIFEST.json (DETERMINISTIC) ==="
OUT="${OUT}" PACK="${PACK}" python3 - <<'PY'
import hashlib, json, os
out = os.environ["OUT"]
pack = os.environ.get("PACK", "PETCARE-PH11-CLOSURE")
records = []
