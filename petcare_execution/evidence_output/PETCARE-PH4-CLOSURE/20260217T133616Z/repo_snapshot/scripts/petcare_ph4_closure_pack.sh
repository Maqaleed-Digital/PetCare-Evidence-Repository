...
grep -qE "^OK$" "${UNITTEST_LOG}" || true
echo "UNITTEST: PASS"
echo ""

echo "STEP 2: START API SERVER (uvicorn)"
SERVER_LOG="${OUT}/uvicorn.log"
PID_FILE="${OUT}/uvicorn.pid"

set +e
PORT="${SMOKE_PORT}" HOST="${HOST}" bash "scripts/run_api.sh" serve >"${SERVER_LOG}" 2>&1 &
UV_PID="$!"
set -e
...
echo "STEP 3: SMOKE"
PORT="${SMOKE_PORT}" HOST="${HOST}" bash "scripts/run_api.sh" smoke 2>&1 | tee "${SMOKE_LOG}"
echo "SMOKE: PASS"
echo ""
