...
echo "=============================================="
echo "PetCare PH65 CLOSURE PACK"
...
echo ""
echo "=== STEP 1: GENERATE (WRITE) — MUST BE IDEMPOTENT ==="
...
echo ""
echo "=== STEP 2: GENERATE (CHECK) — MUST REPORT NO DRIFT ==="
python3 "${REPO_ROOT}/scripts/petcare_verification_index_generate.py" --check --ts_utc "${TS_UTC}" \
  | tee "${OUT}/logs/index_generate_check.log"
...
echo ""
echo "=== STEP 4: QUORUM GUARD (MUST PASS) ==="
...
snap=(
  ...
  "scripts/petcare_verification_index_quorum_guard.sh"
  "scripts/petcare_ci_gates.sh"
  "scripts/petcare_ph65_closure_pack.sh"
)
for f in "${snap[@]}"; do
