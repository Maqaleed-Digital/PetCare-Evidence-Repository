...
echo "=== SNAPSHOT (PH9-RELEVANT FILES) ==="
REQ_FILES="
FND/security/policy.json
FND/security/policy_guard.py
scripts/petcare_audit_verify.py
ops/ph9_gate_registry.json
scripts/petcare_ph9_enforce_gates.py
TESTS/test_ph9_policy_guard.py
scripts/petcare_ph9_closure_pack.sh
"
...
echo "=== ENFORCE GATES (MUST PASS FOR TRUST-GRADE CLOSURE) ==="
python3 "scripts/petcare_ph9_enforce_gates.py" "ops/ph9_gate_registry.json" 2>&1 | tee "${OUT}/ops/gate_enforce.txt"

echo "=== CREATE MANIFEST.json (DETERMINISTIC) ==="
OUT="${OUT}" PACK="${PACK}" python3 - <<'PY'
import hashlib, json, os
out = os.environ["OUT"]
pack = os.environ.get("PACK", "PETCARE-PH9-CLOSURE")
records = []
