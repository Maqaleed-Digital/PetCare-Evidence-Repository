...
echo "=============================================="
echo "PetCare PH10 CLOSURE PACK"
echo "pack=${PACK}"
...
echo "=== SNAPSHOT (PH10-RELEVANT FILES) ==="
REQ_FILES="
FND/security/policy_guard.py
FND/security/flask_guard.py
FND/CODE_SCAFFOLD/app.py
FND/CODE_SCAFFOLD/api/routes_platform_admin.py
TESTS/test_ph10_trust_enforcement.py
ops/ph10_gate_registry.json
scripts/petcare_ph10_enforce_gates.py
scripts/petcare_ph10_closure_pack.sh
"
...
echo "=== ENFORCE GATES ==="
python3 "scripts/petcare_ph10_enforce_gates.py" "ops/ph10_gate_registry.json" 2>&1 | tee "${OUT}/ops/gate_enforce.txt"

echo "=== CREATE MANIFEST.json (DETERMINISTIC) ==="
OUT="${OUT}" PACK="${PACK}" python3 - <<'PY'
import hashlib, json, os
out = os.environ["OUT"]
pack = os.environ.get("PACK", "PETCARE-PH10-CLOSURE")
records = []
