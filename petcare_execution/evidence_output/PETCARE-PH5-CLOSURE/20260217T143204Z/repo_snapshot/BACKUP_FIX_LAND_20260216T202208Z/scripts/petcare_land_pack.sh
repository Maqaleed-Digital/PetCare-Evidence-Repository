...
compile_targets=()
if [ -f "${ROOT}/FND/CODE_SCAFFOLD/app.py" ]; then
  echo "compile: app.py"
  compile_targets+=("${ROOT}/FND/CODE_SCAFFOLD/app.py")
fi

if [ -d "${ROOT}/FND/CODE_SCAFFOLD" ]; then
  echo "compile: scaffold python files (13)"
  "$PYBIN" -m compileall -q "${ROOT}/FND/CODE_SCAFFOLD" >/dev/null
fi

if [ -d "${ROOT}/TESTS" ]; then
  "$PYBIN" -m compileall -q "${ROOT}/TESTS" >/dev/null || true
fi
